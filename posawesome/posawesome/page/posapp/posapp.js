...
const posaTranslations = {
  ar: {
    ...
    "POS Closing Shift {} against {} between selected period": "إقفال وردية نقطة البيع {} مقابل {} خلال الفترة المحددة",
    ...
    "POS Profile {} does not belongs to company {}": "ملف تعريف نقطة البيع {} لا ينتمي إلى الشركة {}",
    ...
    "No invoices found. Try different search criteria.": "لم يتم العثور على فواتير. جرب معايير بحث مختلفة",
    ...
    "Always fetch items from server (ignore local cache)": "جلب الأصناف دائماً من الخادم (تجاهل التخزين المحلي)",
    ...
    "Error submitting invoice No response from server": "خطأ في إرسال الفاتورة لا يوجد استجابة من الخادم",
    ...
    "Payment name not found. Cannot open print view.": "لم يتم العثور على اسم الدفع. لا يمكن فتح عرض الطباعة.",
    ...
    "Please request phone payment or use another payment method": "يرجى طلب الدفع عبر الهاتف أو استخدام طريقة دفع أخرى",
    ...
    "Position the QR code or barcode within the scanning area": "ضع رمز QR أو الباركود داخل منطقة المسح",
    ...
    "Secure context (HTTPS) required for camera access.": "يتطلب الوصول إلى الكاميرا سياق آمن (HTTPS).",
    ...
    "These invoices will be synced when connection is restored": "سيتم مزامنة هذه الفواتير عند استعادة الاتصال",
    ...
    "Warning High cache usage may affect performance.": "تحذير الاستخدام العالي للذاكرة المخبأة قد يؤثر على الأداء.",
...
// =============================================================================
// LANGUAGE SYSTEM - Dynamically load from POS Profile
// =============================================================================

// Global language variable (will be set from POS Profile)
let posa_language = "en"; // Default to English
...
// Listen for POS Profile loaded event to update language
window.addEventListener("posProfileLoaded", function (event) {
  const pos_profile = event.detail.pos_profile;

  // Read language from POS Profile (default to 'en' if not set)
  const new_language = pos_profile.posa_language || "en";

  // Only update if language changed
  if (new_language !== posa_language) {
    console.log(`[POS Translation] Switching language from ${posa_language} to ${new_language}`);
    posa_language = new_language;

    // Update window.__messages for Vue components
    window.__messages = posaTranslations[posa_language] || {};

    console.log(`[POS Translation] Language set to: ${posa_language}`);
  }
});
